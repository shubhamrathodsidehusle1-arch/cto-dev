...
class CallbackTask(Task):
    ...
    def on_failure(
        ...
        """
        logger.error(
            "Task failed", task_id=task_id, error=str(exc), args=args, kwargs=kwargs
        )
    ...
    def on_retry(
        ...
        """
        logger.warning(
            "Task retry", task_id=task_id, error=str(exc), args=args, kwargs=kwargs
        )
        ...
        try:
            ...
            async def update():
                ...
                await update_job_status(
                    db=db, job_id=job_id, status=status, error_message=error_message
                )
    ...
    try:
        ...
        async def process():
            ...
            if healthy_providers:
                ...
                if (
                    selected_provider.metadata
                    and "models" in selected_provider.metadata
                    and selected_provider.metadata["models"]
                ):
        ...
        if self.request.retries < self.max_retries:
            ...
            logger.info(
                "Retrying job", job_id=job_id, retry_count=self.request.retries + 1
            )
