...
async def test_provider(test_request: ProviderTestRequest) -> ProviderHealthResponse:
    ...
    existing_health = await db.providerhealth.find_unique(
        where={"provider": test_request.provider}
    )
    ...
    try:
        ...
        logger.error(
            "Provider test failed", provider=test_request.provider, error=str(e)
        )
        status = "unhealthy"
        error_message = str(e)
